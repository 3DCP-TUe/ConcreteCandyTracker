...
%
% This function calculates key statistical characteristics of step-up
% response curves, including mean, variance, standard deviation, and selected
% percentiles, and returns them in a table suitable for analysis. All time
% quantities are handled as MATLAB durations.
%
% Syntax: properties = get_step_up_properties(times, responses)
%
% Inputs:
%   times      - Vector of durations specifying the step-up time for each
%                response (one element per table in RESPONSES)
%   responses  - Cell array of step-up response tables; each table must contain:
...
%   - Responses are assumed normalized on [0, 1].
%   - Percentiles are reported as the first time where value > p
%     (e.g., p50 is the time the response rises above 0.5).
%   - Variance is constrained to be non-negative before square-rooting to
%     compute the standard deviation.
%
% Example:
%   resp = get_normalized_step_up_response(seconds(0), data, ...
%           [seconds(0) seconds(120)], [seconds(0) seconds(10)], [seconds(90) seconds(120)]);
%   properties = get_step_up_properties(seconds(0), {resp});
